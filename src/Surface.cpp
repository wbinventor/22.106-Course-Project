...
/**
 * Moves a neutron to either the left or right bordering Region1D
 * based on the neutron's trajectory. If this Surface has VACUUM
 * boundary conditions, it instead kills the neutron
 */
void Surface::moveNeutron(neutron* neutron) {

	log_printf(DEBUG, "Inside Surface moveNeutron method");
	log_printf(DEBUG, "Moving neutron with x = %f, mu = %f",
								neutron->_x, neutron->_mu);

	/* If the neutron is not on the Surface, print error message */
	if (!onSurface(neutron->_x))
		log_printf(ERROR, "Cannot move a neutron with x = %f off of "
				"Surface x = %f since it is not on the Surface",
				neutron->_x, _x);
		...
		/* If the surface is reflective */
			...
			/* Figure out which region to put neutron in */
			if (neutron->_mu <= 0)
				_left_region->moveNeutron(neutron);
			else
				_right_region->moveNeutron(neutron);
		}
