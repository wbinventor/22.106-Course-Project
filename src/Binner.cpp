...
void Binner::setBinEdges(float* edges, int num_bins) {
	...
	_tally_acc = new double[num_bins*NUM_THREADS];
	...
	for (int i=0; i < _num_bins; i++) {
		...
		for (int j=0; j < _num_threads; j++)
			_tally_acc[i*NUM_THREADS+j] = 0.0;
	}
...
void Binner::tally(int thread_num, float sample) {
	...
	if (bin_index >= 0 && bin_index < _num_bins) {
		_tally_acc[bin_index*NUM_THREADS+thread_num]++;
		omp_set_lock(&_lock);
		_acc_indices.insert(bin_index);
 ...
 */
void Binner::weightedTally(int thread_num, float sample, float weight) {
	...
	if (bin_index >= 0 && bin_index < _num_bins) {
		_tally_acc[bin_index*NUM_THREADS+thread_num] += weight;
		omp_set_lock(&_lock);
		_acc_indices.insert(bin_index);
		omp_unset_lock(&_lock);
	}
...
//		for (int j=0; j < _num_threads; j++) {
//			_tallies[i] += _tally_acc[i][j];
//			_tallies_squared[i] += (_tally_acc[i][j] * _tally_acc[i][j]);
//			_tally_acc[i][j] = 0.0;
//		}
//	}
	std::set<int>::iterator iter;
	int i;
	for (iter=_acc_indices.begin(); iter != _acc_indices.end(); ++iter) {
		i = *iter;

		for (int j=0; j < _num_threads; j++) {
			_tallies[i] += _tally_acc[i*NUM_THREADS+j];
			_tallies_squared[i] += (_tally_acc[i*NUM_THREADS+j] * _tally_acc[i*NUM_THREADS+j]);
			_tally_acc[i*NUM_THREADS+j] = 0.0;
		}
		_acc_indices.clear();
	}
