...
void Region1D::clearBinners() {
	...
	/* Then remove the binners from this Region1D */
	_bin_sets.clear();
}
			...
			for (iter3 = _bin_sets.begin(); iter3 != _bin_sets.end(); ++iter3){
				...
				if (strcmp(isotopes, "all") == 0) {
					...
					if (curr_bin->getTallyType() == FLUX_SPATIAL)
						curr_bin->weightedTally(curr->_thread_num,
								curr->_x, curr->_weight / sigma_t);

					/* Energy flux tally */
					else if (curr_bin->getTallyType() == FLUX_ENERGY)
						curr_bin->weightedTally(curr->_thread_num,
								curr->_energy, curr->_weight / sigma_t);

					/* Spatial capture rate tally */
					else if (curr_bin->getTallyType()==CAPTURE_RATE_SPATIAL)
						curr_bin->weightedTally(curr->_thread_num,
								curr->_x, curr->_weight *
...
/**
 * This method contains all of the neutron physics for the simulation
 * It moves a neutron by sampling its path traveled and either collides
 * it with an isotope in the region, or places them on one of the bounding
 * surfaces
 * @return whether or not the neutron is still alive
 */
bool Region1D::moveNeutron(neutron* neutron) {

	log_printf(DEBUG, "Inside moveNeutron method for region %s", _region_name);
	...
	/**************************************************************************
	 ***************  TWO-REGION PIN CELL COLLISION PROBABILITY  **************
	 *************************************************************************/

//	if (_region_type == TWO_REGION_PIN_CELL) {
//
//
//		// PROBLEM is that this is occurring each time particle is
//
//		/* find index into the material's energy grid for this neutron */
//		int energy_index = _material->getEnergyGridIndex(neutron->_energy);
//
//		float p_ff = computeFuelFuelCollisionProb(energy_index);
//		float p_mf = computeModeratorFuelCollisionProb(energy_index);
//		float test = float(rand()) / RAND_MAX;
...
//			/* If test is larger than p_ff, move to moderator */
//			if (test > p_ff) {
//				_other_region->moveNeutron(neutron);
//				return;
//			}
//		}
//
...
//	}
	...
	log_printf(DEBUG, "Neutron x = %f, mu = %f, energy = %f", neutron->_x,
												neutron->_mu, neutron->_energy);

	/* Compute total sigma_t from all isotopes */
	energy_index = _material->getEnergyGridIndex(neutron->_energy);
	sigma_t = _material->getTotalMacroXS(energy_index);

	/* Compute path length and the new x position of this neutron */
	path_length = -log(float(rand()) / RAND_MAX) / sigma_t;
	new_x = neutron->_x + neutron->_mu * path_length;
	...
	/* Figure out which isotope the neutron collided in */
	isotope = _material->sampleIsotope(neutron->_energy);

	log_printf(DEBUG, "Neutron collided in isotope: %s",
									isotope->getIsotopeType());

	/* Figure out the collision type */
	collision_type = isotope->getCollisionType(neutron->_energy);
	...
	for (iter3 = _bin_sets.begin(); iter3 != _bin_sets.end(); ++iter3){
		...
		if (strcmp(isotopes, "all") == 0) {
			...
			/* Spatial flux tally for all isotopes */
			if (curr_bin->getTallyType() == FLUX_SPATIAL)
				curr_bin->weightedTally(neutron->_thread_num,
						neutron->_x, neutron->_weight / sigma_t);

			/* Energy flux tally */
			else if (curr_bin->getTallyType() == FLUX_ENERGY)
				curr_bin->weightedTally(neutron->_thread_num,
						neutron->_energy, neutron->_weight / sigma_t);

			/* Spatial capture rate tally */
			else if (curr_bin->getTallyType()==CAPTURE_RATE_SPATIAL)
				curr_bin->weightedTally(neutron->_thread_num,
						neutron->_x, neutron->_weight *
				_material->getCaptureMacroXS(energy_index) / sigma_t);

			/* Energy absorption rate tally */
			else if (curr_bin->getTallyType()==CAPTURE_RATE_ENERGY)
				curr_bin->weightedTally(neutron->_thread_num,
						neutron->_energy, neutron->_weight *
				_material->getCaptureMacroXS(energy_index) / sigma_t);
			...
			/* Spatial diffusion rate tally */
				...
				curr_bin->weightedTally(neutron->_thread_num,
						neutron->_x, neutron->_weight *
					1.0 / (3.0 *_material->getTransportMacroXS(neutron->_energy)
														* sigma_t));
	...
	if (_use_implicit_capture) {
		...
		if (_use_implicit_capture) {
			...
			/* Sample a isotope with a scattering cross-section */
			while (isotope->getScatterXS(neutron->_energy) == 0)
				isotope = _material->sampleIsotope(neutron->_energy);

			while (collision_type == CAPTURE)
				collision_type =
							isotope->getCollisionType(neutron->_energy);
		}
		...
		/* Reduce weight by sigma_a / sigma_t */
		neutron->_weight *= (1.0 - (sigma_a / sigma_t));

		log_printf(DEBUG, "New weight = %f", neutron->_weight);
	}

	/* Forced collision variance reduction */
	//FIXME: cannot currently be used for this method
	else if (_use_forced_collision) {

		log_printf(ERROR, "Forced collision for the moveNeutron method"
...
//		float d;
//		if (neutron->_mu <= 0)
//			d = (neutron->_x - _left_surface->getX()) / neutron->_mu;
//		else
//			d = (_right_surface->getX() - neutron->_x) / neutron->_mu;
//
//		/* Compute exponential term in forced collision prob dist */
//		float px = exp(-sigma_t * d);
//
...
//		 * in the region along the collided neutron's trajectory */
//		neutron* new_neutron = initializeNewNeutron();
//		new_neutron->_energy = neutron->_energy;
//		new_neutron->_mu = neutron->_mu;
//		new_neutron->_weight = neutron->_weight * px;
//
//		/* Figure out which surface to put uncollided neutron on */
//		if (neutron->_mu <= 0)
//			_left_surface->addNeutron(new_neutron);
...
//
//		/* Update collided neutron's properties */
//		neutron->_x += -neutron->_mu * log(1.0 - float(rand()/RAND_MAX)
//										* (1.0 - px)) / sigma_t;
//		neutron->_weight *= (1.0 - px);
	}
	...
	/* Check if collision was general scattering */
		...
		if (isotope->getScatterAngleType() == ISOTROPIC_LAB) {
			float A = float(isotope->getA());
			float energy = neutron->_energy * (1.0 -
				(1.0 - isotope->getAlpha())*(float(rand())/RAND_MAX));
			float mu_cm = ((2 * energy / neutron->_energy) -
							isotope->getAlpha() - 1.0) / (1.0 -
												isotope->getAlpha());
			neutron->_mu = (1.0 + A*mu_cm) / sqrt(A*A + 1.0 + 2*mu_cm*A);

			if (neutron->_energy < 4 && isotope->usesThermalScattering())
				neutron->_energy =
				isotope->getThermalScatteringEnergy(neutron->_energy);
			else
				neutron->_energy = energy;
		}
		...
		/* Isotropic in CM */
			...
			float mu_l = (1.0 + A*mu_cm) / (sqrt(A*A + 2*A*mu_cm+1.0));
			float phi = (float(rand()) / RAND_MAX) * 2.0 * M_PI;
			neutron->_mu = neutron->_mu * mu_l +
					sqrt(1.0 - neutron->_mu * neutron->_mu) *
					sqrt(1.0 - mu_l * mu_l) * sin(phi);
			...
			if (neutron->_energy < 4 && isotope->usesThermalScattering())
				...
				neutron->_energy *= (A*A + 2*A*mu_cm + 1.0) /
													((A+1.0)*(A+1.0));
		}
	...
	/* Check if collision type was inelastic scattering */
		...
		/* Update the neutron energy */
		neutron->_energy =
				isotope->getInelasticScatterEnergy(neutron->_energy);
	...
	/* Check if collision type was elastic scattering */
		...
		if (isotope->getElasticAngleType() == ISOTROPIC_LAB) {
			...
			//CORRECTION: xs only extend to 1E-5
			if (neutron->_energy < 1E-5)
				neutron->_energy = 1.1E-5;
	...
	/* Play Russian Roulette with neutron */
	if (neutron->_weight < _weight_low && !_use_forced_collision) {
		if (playRussianRoulette(neutron)) {
			log_printf(DEBUG, "Killing neutron");
			delete neutron;
			return false;
		}
...
//	 *****************************************************************/
//	/* If the new_x crossed the left surface, place it on the surface */
//	if (_left_surface != NULL && neutron->_x < _left_surface->getX())
//		return true;
//
//	/* If the new_x crossed the right surface, place it on the surcace */
//	else if (_right_surface != NULL && neutron->_x > _right_surface->getX())
//		_right_surface->moveNeutron(neutron);
//
//	/* The neutron remains in this region so we move it again */
//	else
//		moveNeutron(neutron);
	return true;
}
